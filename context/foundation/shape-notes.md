...
checkpoint:
  ...
  gray_areas_resolved:
    - topic: "AI correction vs. flow priority"
      decision: "Natural mid-conversation corrections + major mistake summary at end of session"
    - topic: "Session types"
      decision: "Two modes: topic-based (AI stays on topic) and free conversation (no constraint)"
    - topic: "Voice storage"
      decision: "Voice audio never stored on server — privacy non-negotiable"
    - topic: "Product type"
      decision: "Web app — must work on mobile browser"
    - topic: "Language constraint"
      decision: "AI communicates in English only — never switches to other languages"
    - topic: "Timeline"
      decision: "3 weeks after-hours; web app + AI API (OpenAI/Claude or similar)"
  frs_drafted: 18
...
The learner creates a lesson by filling a template (name, subject, conversation goal, optional vocabulary). This template is injected as the AI's system prompt for every session under that lesson — it defines what territory the AI stays in and what it should introduce. The AI never drifts outside this boundary during a topic-based session.
...
Single user; no auth; data lives on-device only. No login system. No multi-user separation. This is explicitly a one-person application.

## Success Criteria

### Primary
- She can open the app, pick a lesson topic (or start a free conversation session), and have a real back-and-forth voice conversation with an AI teacher that stays on topic, corrects her naturally mid-conversation, and at the end delivers a summary of major mistakes plus suggested topics to explore for improvement.
